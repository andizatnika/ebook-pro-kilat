...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const handleCreateClick = async (e: React.MouseEvent) => {
    ...
    try {
        await onNewEbook();
        // NOTE: If successful, App.tsx will switch component step, causing this to unmount.
        // So we don't need to manually reset 'creating' if unmounted.
    } catch (err) {
        console.error("Error creating ebook from dashboard:", err);
        alert("Terjadi kesalahan saat membuat proyek.");
    } finally {
        // Safe check for unmounted component
        if (isMounted.current) {
             setCreating(false);
        }
    }
  ...
  // Hapus Permanen dari Database dengan Konfirmasi yang Tepat
  const handleDeleteFromDB = async (e: React.MouseEvent, id: string) => {
    ...
    if (window.confirm(message)) {
      try {
        // Pass userId untuk keamanan
        await deleteEbook(id, userId);
        
        // Refresh list lokal
        setEbooks(prev => prev.filter(b => b.id !== id));
        
        // Jika yang dihapus adalah yang sedang aktif di memori
                ...
                <div className="flex flex-col gap-2 w-full sm:w-auto">
                    ...
                    {activeDraft.id && (
                        <button 
                          ...
                          className="whitespace-nowrap flex items-center justify-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer w-full text-sm"
                        >
                  ...
                  <div className="p-6 flex-1">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40 dark:text-red-400 rounded-lg transition-colors border border-red-200 dark:border-red-900/30"
                        title="Hapus Proyek"
                      >
